...
class TypeGenerator extends BaseGenerator {
    ...
    generateAllOfType(name) {
        const schema = this.registry.getSchemaByName(name);
        const { allOf } = schema, rest = __rest(schema, ["allOf"]);
...
class NameProvider {
    ...
    getValidatorName(typeName) {
        return `validate${pascalCase(typeName)}`;
    }
    getTypesImport() {
        return '__T';
    }
    getApiContractImport() {
        return '__A';
    }
}

const cliParser = new argparse.ArgumentParser({
...
paths.forEach((path$$1) => cliParser.addArgument([`--${path$$1}`], {
    required: false,
    dest: path$$1,
    help: `Path for ${path$$1.replace('Path', '')} relative to the generated file (used as import)`,
}));

class CliGenerator {
    constructor() {
        this.args = cliParser.parseArgs();
    }
    readSchema() {
        const file = path.resolve(this.args.file);
        const content = fs.readFileSync(file, 'UTF8');
        const schema = this.parseSchema(path.extname(file), content);
        return schema;
    }
    parseSchema(ext, content) {
